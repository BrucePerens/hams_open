...
def _make_request(method, endpoint, token, error_msg, **kwargs):
    ...
    try:
        if method.upper() == "GET":
            ...
            response = session.post(endpoint, headers=headers, timeout=timeout, **kwargs)
        elif method.upper() == "PUT":
            response = session.put(endpoint, headers=headers, timeout=timeout, **kwargs)
        elif method.upper() == "PATCH":
            response = session.patch(endpoint, headers=headers, timeout=timeout, **kwargs)
        elif method.upper() == "DELETE":
            response = session.delete(endpoint, headers=headers, timeout=timeout, **kwargs)
        else:
...
def purge_urls(urls, token, zone_id):
    ...
    for i in range(0, len(urls), 30):
        chunk = urls[i : i + 30]
        payload = {"files": chunk}
        response = _make_request("POST", endpoint, token, "Cloudflare URL purge API failed for chunk", json=payload, timeout=10)
...
def purge_tags(tags, token, zone_id):
    ...
    for i in range(0, len(tags), 30):
        chunk = tags[i : i + 30]
        payload = {"tags": chunk}
        response = _make_request("POST", endpoint, token, "Cloudflare Tag purge API failed for chunk", json=payload, timeout=10)
        if not response or response.status_code != 200:
            success = False
    return success
...
def ban_ip(ip_address, mode, notes, token, zone_id):
    ...
    response = _make_request("POST", endpoint, token, "Cloudflare WAF IP Ban API failed", json=payload, timeout=10)
...
def unban_ip(rule_id, token, zone_id):
    ...
    response = _make_request("DELETE", endpoint, token, "Cloudflare WAF IP Unban API failed", timeout=10)
    if response and response.status_code == 200:
        return True, "Success"
    return False, "API Error"
...
def get_zone_ruleset(phase, token, zone_id):
    ...
    response = _make_request("GET", endpoint, token, "Cloudflare Ruleset Fetch API failed", timeout=15)
    if response:
        if response.status_code == 404:
            return None
...
def update_zone_ruleset(ruleset_id, payload, token, zone_id):
    ...
    response = _make_request("PUT", endpoint, token, "Cloudflare Ruleset Update API failed", json=payload, timeout=15)
    if response and response.status_code == 200:
...
def create_zone_ruleset(payload, token, zone_id):
    ...
    response = _make_request("POST", endpoint, token, "Cloudflare Ruleset Create API failed", json=payload, timeout=15)
    if response and response.status_code == 200:
...
def create_cfd_tunnel(account_id, token, tunnel_name):
    ...
    response = _make_request("POST", endpoint, token, "Cloudflare Tunnel Create API failed", json=payload, timeout=15)
    if response and response.status_code == 200:
...
def get_cfd_tunnel_token(account_id, token, tunnel_id):
    ...
    response = _make_request("GET", endpoint, token, "Cloudflare Tunnel Token API failed", timeout=15)
    if response and response.status_code == 200:
...
def purge_everything(token, zone_id):
    ...
    response = _make_request("POST", endpoint, token, "Cloudflare Purge Everything API failed", json=payload, timeout=10)
    if response and response.status_code == 200:
        return True
    return False
...
def get_zone_settings(token, zone_id):
    ...
    response = _make_request("GET", endpoint, token, "Cloudflare Get Zone Settings API failed", timeout=10)
    if response:
        if response.status_code == 404:
            return None
        return response.json().get("result")
...
def update_zone_setting(setting_name, value, token, zone_id):
    ...
    payload = {"value": value}
    response = _make_request("PATCH", endpoint, token, "Cloudflare Update Zone Setting API failed", json=payload, timeout=10)
    if response and response.status_code == 200:
...
def list_cfd_tunnels(account_id, token):
    ...
    endpoint = f"https://api.cloudflare.com/client/v4/accounts/{account_id}/cfd_tunnel"
    response = _make_request("GET", endpoint, token, "Cloudflare List Tunnels API failed", timeout=15)
    if response and response.status_code == 200:
        return response.json().get("result", [])
    return []
...
def delete_cfd_tunnel(account_id, token, tunnel_id):
    ...
    response = _make_request("DELETE", endpoint, token, "Cloudflare Delete Tunnel API failed", timeout=15)
    if response and response.status_code == 200:
...
def create_custom_hostname(hostname, token, zone_id):
    ...
    response = _make_request("POST", endpoint, token, "Cloudflare Create Custom Hostname API failed", json=payload, timeout=15)
    if response and response.status_code == 200:
        return True, response.json().get("result", {})
    return False, "API Error"
...
def get_custom_hostname(hostname_id, token, zone_id):
    ...
    response = _make_request("GET", endpoint, token, "Cloudflare Get Custom Hostname API failed", timeout=15)
    if response and response.status_code == 200:
        return True, response.json().get("result", {})
    return False, "API Error"
...
def delete_custom_hostname(hostname_id, token, zone_id):
    ...
    endpoint = f"https://api.cloudflare.com/client/v4/zones/{zone_id}/custom_hostnames/{hostname_id}"
    response = _make_request("DELETE", endpoint, token, "Cloudflare Delete Custom Hostname API failed", timeout=15)
    if response and response.status_code == 200:
