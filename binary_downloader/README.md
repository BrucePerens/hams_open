...
This module is fully compliant with the **Zero-Sudo** mandate and uses micro-privilege service accounts for all operations. It supports raw binaries, Gzipped Tarballs, and ZIP archives.
...
* **Archive Security (Tar/Zip Slip):** Implements strict isolation by utilizing `os.path.basename` for all extracted files during archive extraction (both `.tar.gz` and `.zip`). This guarantees that extracted files are strictly confined to the `hams_bin` directory, regardless of any directory structure within the archive. Symbolic links and hard links within archives are strictly forbidden.
