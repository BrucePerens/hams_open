...
class TestUIPersonas(HamsHttpCase):
    def setUp(self):
        ...
        self.club_op = self.env["res.users"].create(
            {
                "name": "Persona Club Op",
                "login": "personaclub",
                "password": "personaclub",
                "group_ids": [
                    (6, 0, [self.env.ref("base.group_portal").id])
                ],  # club features will be added if needed
            }
        )
    ...
    def test_01_anonymous_persona(self):
        ...
        self.navigate_and_screenshot(
            "/user_websites", prefix="persona_anonymous_user_websites_"
        )

        # Verify a secure feature redirects or 403s
        res = self.url_open("/my/home")
        self.assertTrue(
            res.status_code in [403, 404] or "login" in res.url,
            "Anonymous user should not access /my/home",
        )
    ...
    def test_02_logged_in_portal_user(self):
        ...
        self.navigate_and_screenshot(
            "/user_websites", prefix="persona_portal_user_websites_"
        )
    ...
    def test_03_user_websites_administrator(self):
        ...
        self.navigate_and_screenshot(
            "/user_websites", prefix="persona_siteadmin_user_websites_"
        )
    ...
    def test_05_odoo_administrator(self):
        ...
        self.navigate_and_screenshot(
            "/user_websites", prefix="persona_admin_user_websites_"
        )
