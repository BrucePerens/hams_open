...
class TestHelpdeskMultiWebsite(HamsTransactionCase):
    ...
    def test_07_ticket_creation_from_context(self):
        ...
        ticket = self.env['hams_helpdesk.ticket'].with_context(website_id=self.website_1.id).create({
            'name': 'Context Website Ticket',
        })
        self.assertEqual(ticket.website_id, self.website_1, "Ticket should inherit website_id from context during creation")
