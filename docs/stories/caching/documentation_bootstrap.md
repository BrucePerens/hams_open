...
2. **Detection**: During module installation or registry loading, the `zero_sudo` module's `_bootstrap_knowledge_docs` method checks for compatible documentation models (like `knowledge.article` or `manual.article`).
