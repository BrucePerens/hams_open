...
2. **Locate Caching Block**: They find the **Caching Service Worker** section ([@ANCHOR: xpath_rendering_caching_settings]).
3. **Trigger Invalidation**: The administrator clicks the **Invalidate Cache Now** button.
4. **Backend Update**:
   - The system retrieves the current `caching.invalidation_version` ([@ANCHOR: test_caching_sudo_params]).
...
6. **Service Worker Update**:
   - The next time any user's browser requests `/sw.js`, the server injects the new version number into the `CACHE_NAME` ([@ANCHOR: caching_sw_serve_route]).
