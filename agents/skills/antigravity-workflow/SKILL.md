...
- If the tests fail, **DO NOT** attempt to read the entire task output or the full Odoo log file, as it will exhaust your context window. Instead, read the pre-filtered summary at `~/tmp/filtered_test.txt` using the `view_file` tool to see only the tracebacks and failures.
...
6. **Deadlock Avoidance & Prioritization:** When there is more than one bug evident, deal with the easiest ones first. If some of those turn out to be difficult, put them "on hold" and deal with easier ones first. The goal is to avoid deadlocking for hours on a problem where the user might be able to set you straight immediately upon returning, and to get as much productive work done as possible while the user is not present.
