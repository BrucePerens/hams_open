...
class TestManualRobustness(HamsHttpCase):
    ...
    def test_04_backend_views_rendering(self):
        ...
        v1 = self.env["knowledge.article"].get_view(
            view_id=self.env.ref("knowledge.view_knowledge_article_list").id,
            view_type="list",
        )
        self.assertIn("parent_id", v1["arch"])
        v2 = self.env["knowledge.article"].get_view(
            view_id=self.env.ref("knowledge.view_knowledge_article_form").id,
            view_type="form",
        )
    ...
    def test_06_honeypot_blocking(self):
        ...
        self.assertEqual(
            ...
            initial_helpful,
            "Honeypot should have blocked the increment.",
        )
