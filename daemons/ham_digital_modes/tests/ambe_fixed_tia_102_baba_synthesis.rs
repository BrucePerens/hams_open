...
//! Cross-checks `ambe::fixed::tia_102_baba::synthesis::SynthesisState` against its floating-point sibling
//! `ambe::float::tia_102_baba::synthesis::SynthesisState`, at this port's documented 40 dB SNR bar (see
...
//!
//! Fixed's own combined PCM is `i64` (see `fixed::tia_102_baba::synthesis`'s own doc comment for why), so
//! comparisons here convert it to `f64` the same way `from_q16` converts any other Q16.16 value.

use ham_digital_modes::ambe::fixed::tia_102_baba::error_estimation::FrameErrorsQ16;
use ham_digital_modes::ambe::fixed::tia_102_baba::synthesis::SynthesisState as FixedSynthesisState;
use ham_digital_modes::ambe::float::tia_102_baba::error_estimation::FrameErrors;
