...
//! Cross-checks `ambe::fixed::ratet27::decode::DecoderState` against its floating-point sibling
//! `ambe::float::ratet27::decode::DecoderState`, decoding the *same* real encoded bits on both sides
//! -- the first true end-to-end (bits-in, PCM-out) fixed-vs-float comparison for this port, one level
//! above `ambe_fixed_ratet27_synthesis.rs`'s own already-decoded-parameters-in comparison.
//!
//! Frames are built the same way `ambe::float::ratet27::decode`'s own inline
...
use ham_digital_modes::ambe::fixed::ratet27::decode::{DecoderState as FixedDecoderState, FrameOutcome as FixedFrameOutcome};
use ham_digital_modes::ambe::float::ratet27::decode::{DecoderState as FloatDecoderState, FrameOutcome as FloatFrameOutcome};
use ham_digital_modes::ambe::float::ratet27::parameter_encoding as float_parameter_encoding;
use ham_digital_modes::ambe::float::ratet27::synthesis::SynthesisState as FloatSynthesisState;
use ham_digital_modes::ambe::float::ratet27::vuv;
use ham_digital_modes::ambe::float::ratet27::{bit_prioritization, encode_code_vectors, tables};
...
/// construction `ambe::float::ratet27::decode`'s own inline `build_synthetic_voiced_frame` test
...
/// bits-to-PCM pipeline shows that neither `ambe_fixed_ratet27_synthesis.rs` (fed already-decoded,
/// already-omega0-matched parameters directly) nor `ambe_fixed_ratet27_reconstruct.rs` (checking only
