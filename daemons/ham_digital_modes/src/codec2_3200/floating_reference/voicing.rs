...
//! every function in this file: `is_voiced()`/`VoicingState` here are
//! genuine, live production code for `codec2_1600::Encoder` (imported
//! directly as `fvoicing` in `codec2_1600/mod.rs`) -- 1600bps has no
//! fixed-point voicing decision of its own to supersede this one the way
//! 3200bps's `is_voiced_fixed` did. The original, fully-`f32` voiced/unvoiced decision (the
//! classic energy + zero-crossing-rate heuristic, Rabiner & Sambur
//! 1975's baseline endpoint/voicing detector), moved here from
//! `codec2_3200::voicing` once `is_voiced_fixed`/`VoicingStateFixed`
//! (which stayed in that module -- production, used by `EncoderFixed`)
//! made this the only remaining caller.
//!
...
/// voiced, updating `state`'s noise-floor tracking. `pub(crate)`, not
/// `pub`: this module's only real caller outside its own tests is
/// `floating_reference::Encoder` (a sibling module), and `codec2_3200::
...
pub(crate) mod tests {
    ...
    pub(crate) fn synthetic_tone(f0_hz: f32, amp: f32, n: usize, sample_rate: f32) -> Vec<f32> {
        (0..n)
            .map(|i| amp * (std::f32::consts::TAU * f0_hz * i as f32 / sample_rate).sin())
            .collect()
    }

    pub(crate) fn white_noise(amp: f32, n: usize, seed: &mut u32) -> Vec<f32> {
        (0..n)
            .map(|_| {
