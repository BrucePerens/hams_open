...
//! frequency and baud rate"): `rtty_demodulate` is a real, working
//! whole-buffer decoder that does real start-bit edge detection (RTTY is
...
//! framing mechanism), but has not been tuned or tested against real,
//! noisy off-air audio, and has no chunk-surviving streaming wrapper yet
//! (the `Psk31Decoder`-style object that would let this run inside the
//! live daemon pipeline, matching `digital_decoder.rs`'s existing
//! pattern) -- real, separate follow-on work, the same scope boundary
//! `psk31_decode_audio`/`psk31_demodulate` had before `Psk31Decoder`
//! wrapped them.
...
/// detection, no chunk-surviving streaming wrapper yet).
pub fn rtty_demodulate(samples: &[i16], mark_hz: f64, sample_rate: u32) -> String {
    ...
    let mut current_figs = false;
    let mut pos = 0usize;
    let mut prev_was_mark = true; // assume idle (MARK) before the signal starts

    while pos + window_len <= samples.len() {
        let this_is_mark = window_is_mark(&samples[pos..pos + window_len], mark_hz, space_hz, sample_rate);
        if prev_was_mark && !this_is_mark {
            // Candidate start-bit edge at `pos`. Sample the start bit's
            // own center first as a real confirmation (must be SPACE) --
            // a bare mark->space transition in the coarse scan can be
            // noise, not a real start bit.
            let bit_at = |bit_index_from_edge: f64| -> Option<bool> {
                let center = pos as f64 + bit_index_from_edge * samples_per_bit;
            ...
            if let Some(start_bit_is_mark) = bit_at(0.5) {
                if !start_bit_is_mark {
                    ...
                    if framing_ok && stop_ok {
                        match code {
                            CODE_LTRS_SHIFT => current_figs = false,
                            CODE_FIGS_SHIFT => current_figs = true,
                            _ => {
                                let c = if current_figs { FIGS_CHARS[code as usize] } else { LTRS_CHARS[code as usize] };
                                if c != '\0' {
                                    out.push(c);
                                }
                            }
