...
| Oracle D (decoder only) | JMBE, `DSheirer/jmbe`, commit `8ba19fba7449482cb88efcf12eb345ff3ea17dc0`, GPL-3 | `codec/src/main/java/jmbe/codec/imbe/` does contain a full IMBE decoder (and `ambe/`, `ambeplus/`), and no encoder. **Not built**: it needs Gradle (the wrapper downloads a distribution) and slf4j, which the downloads authorisation did not cover, and mbelib, `imbe.rs` and oracle C already gave three decoders to compare with. Its source was read for the damaged-frame policy only (below). |
...
before the newest sample it has seen, and its initial pitch estimate runs on a causally low-pass filtered copy (a
...
| 10 | **Oracle: initial-pitch window 10 samples earlier than the refinement window** (causal pitch low-pass filter, see Alignment). | **Oracle deviates from Fig. 8**; not replicated. | Frame pitch agreement 0.862 with the centres coinciding versus 0.991 with the oracle's own 10-sample stagger. |
...
* **JMBE** as a running second decoder: needs Gradle and slf4j (not authorised) or a JDK plus hand-built classpath;
  little to gain over three decoders that already agree.
