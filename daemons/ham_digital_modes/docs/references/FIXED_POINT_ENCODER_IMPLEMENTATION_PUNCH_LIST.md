...
## `exp2_lut` interpolation, and now `lpc.rs`'s `acos_lut_fixed`, are all genuinely integer (see their
## own rows below); the `Wo`/LSP-Hz quantizers and the pitch estimator (`nlp.rs`) are still `f32` and not
## yet started. This file is the ground truth for what's actually done -- re-derive from here against
## the real code, not from a chat summary, before ever reporting this "complete."
...
| `quantise.rs` (`encode_lsps_delta_scalar`) | Full `f32` | Fixed-point port + validation | **NOT STARTED -- no longer blocked on `acos()` (see the `acos_lut_fixed` row above), but not yet done**: earlier scoping in this file said this had "no real payoff" alongside `encode_wo`, which was wrong (advisor caught it) -- its real blocker was the `.acos()` call, which is now a fixed-point LUT. What remains is porting `lsp_dim_nearest_level`'s own linear-scan-over-32-levels quantizer arithmetic itself to fixed-point (straightforward, no transcendental involved) and deciding whether to keep the `HZ_PER_RAD` conversion in `f32` or fold it into the LUT's own output format -- a real, doable, but still separate task from closing the `acos()` gap. |
...
`encode_energy`/`decode_energy` now ride `log2_lut`'s, `exp2_lut`'s, and (for `encode_lsps_delta_scalar`
specifically) `acos_lut_fixed`'s real fixes for free, but the quantizers' own arithmetic isn't ported),
and the fixed-point FFT (`nlp.rs`, still the largest single remaining item). Real progress landed on the
entire LPC-analysis chain (windowing through LSP frequencies, including `acos()`) plus all three
`log2_lut`/`exp2_lut`/`acos_lut_fixed` LUTs' own interpolation, but the `Wo`/LSP quantizers and the
pitch estimator haven't been touched.
