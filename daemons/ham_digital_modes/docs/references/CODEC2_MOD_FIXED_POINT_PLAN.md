...
| `postfilter()` | `e = 10*log10f(sum(model->A[m]^2)/L)`, `bg_est` (EMA), `thresh = POW10F(...)` | dB-domain accumulator, narrow real range in practice (compared against `BG_THRESH=40.0`) | **Not independently tested against real data -- flagged honestly rather than left implicitly validated by proximity to the rows above.** Every other log-domain stage this proposal has actually tested (`aks_to_mag2`, `nlp()`, `encode_energy`, once its own real methodology error was caught and fixed) confirms the same 8-bit log2/exp2 LUT generalizes well, so the reasoned expectation here is favorable. But this stage still needs `model->A[]` captured directly to actually test (not yet done -- `aks_to_mag2`'s own `A2[]`/`gain` were captured, not the final per-harmonic amplitudes `postfilter()` consumes), and this proposal's own `encode_energy` episode is a concrete, recent reminder that a reasoned expectation and a verified one are not the same claim, even when every prior result points the same direction. Left as the one remaining real gap in this proposal's own validation coverage. |
