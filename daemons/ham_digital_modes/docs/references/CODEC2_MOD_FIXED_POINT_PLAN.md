...
| `encode_energy`/`decode_energy` | `e` (linear) / `e_db` | `e_db` in `[E_MIN_DB, E_MAX_DB] = [-10, 40]` dB, linear `e` in `[10^-1, 10^4]` -- a real 5-decade span | **Validated 2026-09-04, with a real, non-obvious result: the 8-bit log2/exp2 LUT already validated for `aks_to_mag2`/`nlp()` is NOT accurate enough here.** Tested by reusing that exact LUT (`log10(x) = log2(x)*log10(2)`) against 5078 real `E` values (`aks_to_mag2`'s own `E`, already captured) feeding `encode_energy`'s real 5-bit (`E_BITS=5`, 32-level) quantizer: **8.6% of frames picked a different quantizer index than the exact float reference at 8-bit LUT resolution** -- not because the LUT is inaccurate in absolute terms (`decode_energy`'s own reconstruction error stayed ~10^-6 throughout, matching `aks_to_mag2`'s result), but because 32 levels across 50dB is a *coarse* quantizer (~1.56dB/step) that a same-precision LUT error can flip across a real boundary far more often than a stage like `invA2` that only ever compares or accumulates, never buckets into a handful of discrete bins. A resolution sweep (8/9/10/11/12/14/16-bit) showed clean, mostly-monotonic convergence -- 8.6% to 2.3% to 0.71% to 0.12% to 0.06%, settling near a small residual floor (~0.06-0.12%) consistent with real near-exact quantizer-boundary ties any implementation would disagree on. **A real, actionable finding: this stage needs roughly an 11-12 bit LUT, not the 8-bit one reused elsewhere in this codec** -- a genuine counter-example to assuming one LUT resolution choice generalizes across every log-domain stage. |
...
| `postfilter()` | `e = 10*log10f(sum(model->A[m]^2)/L)`, `bg_est` (EMA), `thresh = POW10F(...)` | dB-domain accumulator, narrow real range in practice (compared against `BG_THRESH=40.0`) | **Not independently tested against real data -- flagged honestly rather than left implicitly validated by proximity to the rows above.** Structurally closer to the already-validated `aks_to_mag2`/`nlp()` pattern (a continuous EMA and threshold comparison) than to `encode_energy`'s surprising discrete-quantizer sensitivity found just above -- so the reasoned expectation is that the same 8-bit log2/exp2 LUT suffices here too, unlike `encode_energy`. But `encode_energy`'s own result is exactly why that expectation shouldn't be trusted without running the same real-data test this proposal has now applied to every other log-domain stage: it would need `model->A[]` captured directly (not yet done -- `aks_to_mag2`'s own `A2[]`/`gain` were captured, not the final per-harmonic amplitudes `postfilter()` actually consumes). Left as the one remaining real gap in this proposal's own validation coverage. |
