...
| `nlp()` | notch filter (`mem_x`, `mem_y`, `COEFF=0.95`) and decimating FIR feeding `Fw[]` | **Real measured ranges** (`INSTR_SQ_POSTNOTCH`/`INSTR_SQ_FIR`, corpus-wide min/max/min\|v\|): post-notch `sq[]`: -7.43x10^8 to 1.03x10^9, smallest nonzero \|v\| 0.0033. Post-FIR `sq_fir[]`: -6.69x10^8 to 7.55x10^8, smallest nonzero \|v\| 0.021. Both real, ~12-decade dynamic ranges. | **Validated end-to-end, 2026-09-04.** Built `nlp_fixed()`, a parallel copy of the real `nlp()` with block-floating mantissa quantization inserted at every stage this row flags -- `mem_x`/`mem_y`'s own feedback state (so quantization noise propagates through the real IIR recursion, not just at its output), `sq[i]`'s own notch output, and the FIR's own accumulator -- everything downstream (window, the real vendored `kiss_fftr`, power, argmax, `post_process_sub_multiples`) runs unmodified, real float. Ran both `nlp()` and `nlp_fixed()` side by side on the identical real `Sn[]` snapshot at the identical point in `analyse_one_frame()` (a separate `nlp_t` state, never touching the real reference's own state) across all 5078 real sub-frames, comparing the real, audible output (`best_f0`'s own discrete 3.125Hz bin, not raw float equality -- two bit-different floats landing in the same bin are not a real disagreement). **Result: 0 real bin mismatches at a 15-bit mantissa**, with clean, monotonic convergence from coarser widths (8-bit: 7.7% mismatch/265Hz worst case; 12-bit: 0.47%/178Hz; 13-bit: 0.28%/75Hz; 14-bit: 0.06%/3.1Hz; 15-bit: 0%) -- no bifurcation anywhere in the sweep, the same well-behaved shape as every other block-floating stage in this proposal. **This is the first, and only, fully end-to-end validated stage in the whole codec**: every other validated row tested one closed-form function or comparison in isolation; this one validates the real notch-IIR-state -> FIR -> window -> FFT -> argmax chain together, on real data, against the real reference. |
