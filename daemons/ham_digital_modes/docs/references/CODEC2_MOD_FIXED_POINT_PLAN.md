...
| `nlp()` | notch filter (`mem_x`, `mem_y`, `COEFF=0.95`) and decimating FIR feeding `Fw[]` | **Real measured ranges** (`INSTR_SQ_POSTNOTCH`/`INSTR_SQ_FIR`, corpus-wide min/max/min\|v\|): post-notch `sq[]`: -7.43x10^8 to 1.03x10^9, smallest nonzero \|v\| 0.0033. Post-FIR `sq_fir[]`: -6.69x10^8 to 7.55x10^8, smallest nonzero \|v\| 0.021. Both real, ~12-decade dynamic ranges. | **The notch/FIR stages themselves validated, 2026-09-04 -- not the whole chain, stated precisely.** Built `nlp_fixed()`, a parallel copy of the real `nlp()` with block-floating mantissa quantization inserted at the notch filter's own feedback state (`mem_x`/`mem_y`, so quantization noise propagates through the real IIR recursion, not just at its output), `sq[i]`'s own notch output, and the FIR's own accumulator. **The window multiply, the real vendored `kiss_fftr`, and the power computation ran in exact float, unquantized** -- the chain was *traversed* end-to-end, but only the notch/FIR stages were actually under test; a real port would also need to quantize the window multiply and run KissFFT in its own `FIXED_POINT` mode (this table's own KissFFT row, below), neither of which this test covers. Ran both `nlp()` and `nlp_fixed()` side by side on the identical real `Sn[]` snapshot at the identical point in `analyse_one_frame()` (a separate `nlp_t` state, never touching the real reference's own) across all 5078 real sub-frames, comparing the real, audible output (`best_f0`'s own discrete 3.125Hz bin, not raw float equality -- two bit-different floats landing in the same bin are not a real disagreement). **Result: quantizing just the notch/FIR stages and running everything else in float gives 0 real bin mismatches at a 15-bit mantissa**, with clean, monotonic convergence from coarser widths (8-bit: 7.7% mismatch/265Hz worst case; 12-bit: 0.47%/178Hz; 13-bit: 0.28%/75Hz; 14-bit: 0.06%/3.1Hz; 15-bit: 0%) -- no bifurcation anywhere in the sweep. (This 15-bit figure and the FFT-power row's own 18-bit figure aren't competing precision requirements for the same quantity -- they're different quantization points in the same chain, not directly comparable.) This is the most complete test in this proposal -- a real notch-IIR-state -> FIR chain validated against real data, not a closed-form function tested in isolation -- but it is not a full end-to-end fixed-point validation of `nlp()`. |
| `nlp()` | pitch period / `best_f0` | Bounded, `P_MIN..P_MAX` = 20-160 samples, ~50-400Hz | **Not independently tested** -- the bound itself is a real, construction-guaranteed algorithmic fact (the bin-index search range is hard-limited to `[P_MIN, P_MAX]`), not an empirical claim needing verification, but no fixed-point representation of this quantity has actually been built or tested against real data. |
| `autocorrelate()` | `R[0..10]` | **Real measured range (see "Real measured ranges" below): 4.15x10^-6 to 8.60x10^5 across real speech** -- a real ~10^11 span, not just a bounded maximum | **Two separable concerns, both now checked against real data, 2026-09-04.** (1) The *cross-frame* 11-decade span (quiet vs. loud speech) is the real risk, and needs block-floating -- already validated indirectly: Levinson-Durbin's own block-floating treatment of `e` (derived from `R[]`) was tested against real captured `R[]` data and works at 16+ bits (the clamp bifurcation is separate and unaffected by `R[]`'s own representation width). (2) The *within-frame accumulation itself* (`Rn[j] = sum of Wn[i]*Wn[i+j]` over 320 terms) needs no special treatment: verified directly against all 2532 real captured `R[]` vectors that `\|R[j]\| <= R[0]` holds with zero violations (the real Cauchy-Schwarz bound autocorrelation always satisfies), so a single per-frame accumulator sized for `R[0]`'s own real measured range (needs ~20 bits) covers every `R[j]` term too -- an ordinary wide accumulator, not a new DSP design problem. |
...
| `lpc_to_lsp()` / `cheb_poly_eva()` | `x` (search variable) | Exactly `[-1, 1]` by construction (Chebyshev domain) | **Partially validated as a side effect of the `P[]`/`Q[]` row above**: that test's dense 4001-point sweep of `x` across `[-1,1]` used a real Q2.29 fixed-point `x`/`T` register throughout, so `x`'s own representation width has real evidence behind it (zero sign mismatches). Not tested independently of that specific evaluation context, and not tested against `x` values the real adaptive bisection search itself would generate (the dense sweep is exhaustive over the domain, not a replay of real bisection state). |
| `lpc_to_lsp()` | `P[]`/`Q[]` polynomial coefficients, `psum` | **Validated 2026-09-04, against a real fixed-point *evaluation*, not just quantized coefficients fed through a `double` recurrence (that weaker first test was caught and corrected before this row was finalized -- see the write-up below).** Derived from LPC coefficients (`a[i] + a[11-i]`), real measured max \|coeff\| = 77.17 (see "Real measured ranges"). Combined with the bisection's own sign-only sensitivity (this table's next row), tested Q8.23 coefficients (8 integer bits, sized to the real max) together with a Q2.29 Chebyshev `T` register (the part of `cheb_poly_eva` that actually recurses and compounds error across 5 steps -- `\|T\|<=1` by construction, so 2 integer bits give real margin) against a dense 4001-point `x` sweep across all 2532 real captured coefficient sets (20.26 million total sign comparisons): **zero sign mismatches** at this realistic register width. Confirmed the test genuinely exercises `T`'s own quantization, not just the coefficients: deliberately coarsening `T` alone (still `x`-bounded, just fewer fractional bits) produced 0.002%, 0.038%, and 0.59% mismatch rates at 16, 20, and 24 integer bits respectively -- a real, monotonic sensitivity to `T`'s own precision that the zero-mismatch result specifically rules out at the realistic Q2.29 width. | **Low risk, closed.** Q8.23 coefficients with a Q2.29 `T` register are validated against real data with zero sign errors under a real fixed-point evaluation (not just quantized inputs) -- no open numerical-soundness question remains for this stage. |
...
| LSP frequencies (post-`fast_acosf`) | `freq[j]` | Exactly `[0, pi]` by construction | **Not independently tested** -- the `[0,pi]` bound is a real, construction-guaranteed fact (`acos`'s own range), not an empirical claim, but no fixed-point representation of this quantity has been built or tested against real data. |
| `encode_Wo`/`decode_Wo` | `Wo` | `[W0_MIN, W0_MAX]`, already linearly quantized to 7 bits in the real bitstream | **Not independently tested** -- the reference bitstream's own existing scalar quantizer is real evidence this domain is fixed-point-friendly (it's already a real fixed-point field in production use), but this proposal has not itself built or tested a fixed-point internal representation reusing that quantizer's arithmetic. |
| `encode_energy`/`decode_energy` | `e` (linear) / `e_db` | `e_db` in `[E_MIN_DB, E_MAX_DB] = [-10, 40]` dB, linear `e` in `[10^-1, 10^4]` -- a real 5-decade span | **Validated 2026-09-04, after a real methodology error caught and corrected -- see the write-up below.** The 8-bit log2/exp2 LUT already validated for `aks_to_mag2`/`nlp()` is fine here too: tested against 2539 real *encoder-side* `e` values (the actual `encode_energy(e, E_BITS)` call-site argument in `codec2_encode`, captured directly, not reused from a decoder-side dump) -- **zero quantizer-index mismatches** out of 2539 real frames, decode-side reconstruction error ~10^-6 (matching every other log-domain stage in this proposal). No special-casing needed; the same 8-bit LUT generalizes here after all. |
...
| `postfilter()` | `e = 10*log10f(sum(model->A[m]^2)/L)`, `bg_est` (EMA), `thresh = POW10F(...)` | dB-domain accumulator, narrow real range in practice (compared against `BG_THRESH=40.0`) | **Validated 2026-09-04.** This stage is structurally different from every other log-domain stage tested here: `bg_est` is a real *stateful* EMA carrying across frames (`bg_est = bg_est*(1-BG_BETA) + e*BG_BETA` on unvoiced frames), so a faithful test needed a real temporal replay, not independent per-frame samples -- learned directly from the `encode_energy` mistake above. Captured real `(voiced, L, A[1..L])` for all 5078 real sub-frames *in real temporal order*, then ran two parallel state machines (exact float, and the same 8-bit log2/exp2 LUT used everywhere else) forward through the identical sequence, checking the real, audible decision this stage makes (`A[m] < thresh` -> harmonic phase gets randomized) at all 145,576 real per-harmonic checks across 3516 real voiced frames: **zero decision mismatches**, `bg_est` drift 5.7x10^-6 dB across the whole corpus (no runaway EMA drift), confirmed non-vacuous via a coarse 4-bit LUT (1 real mismatch, 190x more drift). **Why the margin is this forgiving, checked against the real data, not just asserted**: a typical real frame has `A[m]` spanning roughly 0.36 to 70 while `thresh` sits near 39 -- most harmonics are orders of magnitude from the boundary, so only the one or two straddling it can ever flip, which is exactly why even the deliberately-coarse 4-bit LUT only produced a single mismatch. |
