...
| `nlp()` | notch filter (`mem_x`, `mem_y`, `COEFF=0.95`) and decimating FIR feeding `Fw[]` | **Real measured ranges, 2026-09-04** (`INSTR_SQ_POSTNOTCH`/`INSTR_SQ_FIR`, corpus-wide min/max/min\|v\|): post-notch `sq[]`: -7.43x10^8 to 1.03x10^9, smallest nonzero \|v\| 0.0033. Post-FIR `sq_fir[]`: -6.69x10^8 to 7.55x10^8, smallest nonzero \|v\| 0.021. Both real, ~12-decade dynamic ranges, confirming (not just reasoning from) the earlier "huge-range `sq[]` signal" characterization. | **Range measured, but not the same as an end-to-end block-floating validation** -- this is a real number, not a re-derivation from structure, but it stops short of what `aks_to_mag2`/`nlp()`'s power spectrum got: an actual fixed-point simulation compared against the real float reference end-to-end. That would need reimplementing the notch IIR + decimating FIR + window + FFT chain in a parallel fixed-point path (substantially more surface area than every other test in this proposal, which tested closed-form functions or comparisons in isolation) -- left as real, scoped follow-on work, not attempted here. The block-floating recommendation remains reasoned from these real numbers, not independently confirmed the way the downstream `Fw[i].r` stage was. |
