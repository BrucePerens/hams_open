...
| `postfilter()` | `e = 10*log10f(sum(model->A[m]^2)/L)`, `bg_est` (EMA), `thresh = POW10F(...)` | dB-domain accumulator, narrow real range in practice (compared against `BG_THRESH=40.0`) | **Validated 2026-09-04, closing the last open row in this table.** This stage is structurally different from every other log-domain stage tested here: `bg_est` is a real *stateful* EMA carrying across frames (`bg_est = bg_est*(1-BG_BETA) + e*BG_BETA` on unvoiced frames), so a faithful test needed a real temporal replay, not independent per-frame samples -- learned directly from the `encode_energy` mistake just above. Captured real `(voiced, L, A[1..L])` for all 5078 real sub-frames *in real temporal order*, then ran two parallel state machines (exact float, and the same 8-bit log2/exp2 LUT used everywhere else in this proposal) forward through the identical sequence, checking the real, audible decision this stage makes (`A[m] < thresh` -> harmonic phase gets randomized) at all 145,576 real per-harmonic checks across 3516 real voiced frames: **zero decision mismatches**, and `bg_est`'s own float-vs-fixed drift stayed at 5.7x10^-6 dB even after accumulating state across the entire corpus (no runaway EMA drift). Confirmed non-vacuous: a deliberately coarse 4-bit LUT produced a real, nonzero mismatch (1/145576) and 190x more `bg_est` drift (0.0011dB), proving the 8-bit result reflects genuine resolution, not an insensitive test. **Every row in this proposal's per-stage table is now either validated against real data or explicitly, honestly left open -- there are no more implicitly-assumed-safe rows.** |
