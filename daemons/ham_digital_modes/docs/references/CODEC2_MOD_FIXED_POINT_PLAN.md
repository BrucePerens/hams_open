...
| `lpc_to_lsp()` | `P[]`/`Q[]` polynomial coefficients, `psum` | **Validated 2026-09-04.** Derived from LPC coefficients (`a[i] + a[11-i]`), real measured max \|coeff\| = 77.17 (see "Real measured ranges"). Combined with the bisection's own sign-only sensitivity (this table's next row), tested a Q8.23 fixed-point representation (8 integer bits, sized exactly to the real measured max) of real captured `P[]`/`Q[]` against a dense 4001-point sweep of `x` in `[-1,1]` across all 2532 real captured coefficient sets (20.26 million total `cheb_poly_eva` sign comparisons): **zero sign mismatches at Q8.23.** Coarser formats (fewer fractional bits, e.g. Q10.21/Q12.19) showed a handful of mismatches, always within `1.6x10^-8` of an exact zero -- floating-point noise-floor territory, not a real risk margin. | **Low risk, closed.** A real Q8.23 fixed-point format is validated against real data with zero sign errors -- no open numerical-soundness question remains for this stage. |
