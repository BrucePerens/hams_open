...
| `nlp()` | notch filter (`mem_x`, `mem_y`, `COEFF=0.95`) | Simple IIR on the same huge-range `sq[]` signal | **Not independently tested against real data** -- reasoned only, not verified the way the FFT power spectrum above was. `nlp()`'s own validated section explicitly says so: "Not tested here: the notch-filter/decimating-FIR stages feeding into `Fw[]`." The block-floating recommendation is a reasonable inference from the shared `sq[]` signal, not a confirmed result. |
