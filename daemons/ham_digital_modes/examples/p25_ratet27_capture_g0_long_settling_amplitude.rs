...
//! Re-tests `g0`'s amplitude relationship (section 23's gain-quantizer finding, originally captured
//! with only 50 settling frames per amplitude) with a much longer settling period per section 32's
//! adaptive-baseline discovery, to see whether the earlier "big jump then near-plateau" shape
//! sharpens into a cleaner quantizer curve given more time to converge, or stays the same (meaning
//! the original result was already a genuine steady-state reading, not a settling artifact).
//!
//! Usage: `cargo run --release --example p25_ratet27_capture_g0_long_settling_amplitude -- <host:port>`
