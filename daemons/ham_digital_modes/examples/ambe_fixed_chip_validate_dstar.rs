...
//! Live chip validation for `ambe::fixed::dstar::decode`: feeds real recorded speech through the
//! real DVSI chip under D-STAR's own RATEP, and for every resulting real channel frame, confirms the
//! fixed-point `dequantize` tracks the already-chip-validated floating-point sibling within this
//! crate's own documented tolerance -- the same reasoning and structure as
//! `examples/ambe_fixed_chip_validate_ambe_plus_2.rs`, applied to D-STAR.
//!
//! Usage: `cargo run --release --example ambe_fixed_chip_validate_dstar -- <host:port>`
...
use ham_digital_modes::ambe::float::dstar::decode::{classify_b0, DStarDecoderState, DequantizedFrame, FrameKind};
...
fn main() {
    ...
    for path in speech_files {
        ...
        for i in 0..n_frames {
            ...
            wire_bytes.copy_from_slice(&payload[2..2 + 9]);
            let frame = wire_bytes_to_frame(&wire_bytes);
            let parsed = ham_digital_modes::ambe::float::dstar::decode::parse_frame(frame);
            total_frames += 1;
            let raw = ham_digital_modes::ambe::float::dstar::decode::extract_raw_parameters(parsed.d);

            if classify_b0(raw.b0) == FrameKind::Tone {
                continue; // Tone frames: already pure-integer, not this validator's concern.
            }
            speech_frames += 1;

            let float_result = float_decode::dequantize(parsed.d, &mut float_state);
            let fixed_result = fixed_decode::dequantize(parsed.d, &mut fixed_state);
    ...
    const MAX_ACCEPTABLE_BOUNDARY_RATE: f64 = 0.01; // 1% -- real observed rate is far lower (~0.03%).
