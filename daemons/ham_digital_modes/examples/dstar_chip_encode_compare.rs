...
#![allow(dead_code)]
//! Encode-direction comparison of this crate's D-STAR [`Encoder`] against the real chip's encoder on real speech,
...
//! correlation 0.843 against the input; our encode->chip decode reaches 0.901 at analysis offset +160 (rising with
//! offset up to one frame, the decoder's latency), and our encode->our decode 0.981. Pitch index agreement with the
//! chip encoder's own frames is ~46% within 2 steps at the best offsets.
//!
//! Usage: `cargo run --release --example dstar_chip_encode_compare -- [host:port] [wav] [n_frames]`

use ham_digital_modes::ambe::float::dstar::decode::{dequantize, extract_raw_parameters, parse_frame, DStarDecoderState, DequantizedFrame};
