...
//! Live chip validation for `ambe_plus_2::decode`'s `TONE_IDX` decode (DVSI's own documented field,
//! `AMBE-3000R Vocoder Chip Users Manual` Table 103/104 -- see `decode_tone_idx`'s own doc comment):
//! feeds all 16 real ITU-T Q.23 DTMF digit tones, plus 4 representative single (non-DTMF) tones, to
//! the chip under AMBE+2 half-rate with `TD_ENABLE` set. For DTMF, confirms `classify_b0` reports
//! `Erasure` (this rate's real serialization -- not its own spec-defined `Tone` range, see
//! `FrameKind::Erasure`'s own doc comment), `ECMODE_OUT`'s `TONE_FRAME` ground-truth bit reads 1, and
//! `dtmf_digit_from_tone_idx` recovers the exact row/column pair for every one -- straight from the
//! chip's own live response, not the frozen fixtures `decode.rs`'s own unit tests check against. For
//! single tones, confirms `decode_tone_idx` matches DVSI's own documented `round(f0/31.25Hz)`
//! formula. Closes the "no known field decodes it" gap section 40 originally left open for AMBE+2
//! half-rate's tone/DTMF frames, mirroring `examples/ambe_chip_validate_dstar_tone.rs`'s own
...
fn main() {
    ...
    for (ri, &row) in rows.iter().enumerate() {
        for (ci, &col) in cols.iter().enumerate() {
            ...
            for pkt in &pkts {
                ...
                let frame_ok = is_erasure && tone_frame && decoded == Some((ri as u8, ci as u8));
                if !frame_ok {
                    digit_ok = false;
                }
                println!(
                    "  digit {}: b0={} kind={kind:?} TONE_FRAME={} tone_idx={tone_idx:?} decoded={decoded:?} expected=Some(({ri},{ci})) {}",
                    digit_names[ri][ci],
                    raw.b0,
                    tone_frame_bit(pkt),
    ...
    println!("-- single (non-DTMF) tones: confirming DVSI's own round(f0/31.25Hz) formula (Table 104) --");
    // Frequencies chosen from AMBE_CHIP_VALIDATION_FINDINGS.md section 40's own probe run --
    // reliably classified as a tone by the chip (many single frequencies tried are not).
    for &f0_hz in &[250.00f64, 203.12, 395.85, 351.56] {
        let expected = (f0_hz / 31.25).round() as u8;
        let samples = sine_tone(f0_hz, 16000.0);
        let pkts = capture(&sock, &samples);
        ...
        for pkt in &pkts {
            ...
            println!(
                "  f0={f0_hz:.2}Hz: b0={} kind={kind:?} TONE_FRAME={} tone_idx={tone_idx:?} expected=Some(0x{expected:02x}) {}",
                raw.b0,
                tone_frame_bit(pkt),
                if frame_ok { "OK" } else { "MISMATCH" }
            );
    ...
    if all_ok {
        println!(
            "\nPASS: all 16 DTMF digits and 4 single tones, all {CAPTURE_FRAMES} captured frames each, \
             decode correctly via ambe_plus_2::decode::decode_tone_idx (classify_b0=Erasure, \
             TONE_FRAME=1, tone_idx matches DVSI's own documented values)."
        );
    } else {
