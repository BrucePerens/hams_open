...
#![allow(dead_code)]
//! Is the chip's unvoiced-noise excitation deterministic, and does it start from the same state after a reconfiguration?
...
fn main() {
    ...
    configure(&sock, &mut buf);
    let second = decode(&sock, &mut buf, &seq);
    let mut identical = 0;
    ...
    for (i, f) in seq.iter().enumerate() {
        let ours = dec.decode_frame(*f).unwrap().to_vec();
        let (c, lag) = corr_at_best_lag(&first[i], &ours, 40);
        println!("frame {i}: sample-level correlation chip vs ours {c:.3} at lag {lag}");
    }
}
