...
//! Re-test of `p25_ratet27_capture_rms_normalized_pitch_sweep.rs`'s 60-frame-settling pitch sweep,
//! at 300 frames of settling per frequency (matching section 33's proven-sufficient protocol), to
//! resolve whether `u4`'s dither-step-vs-frequency picture (section 34) is a genuine frequency effect
//! or an artifact of that sweep's short settling period -- `ratet27_analyze_u4_dither_by_frequency`'s
//! offline analysis of the original 60-frame data found 3-5 distinct `u4` values at many frequencies
//! (not the clean 2 a settled dither pattern should show), consistent with under-settling rather than
//! a clean quantizer signature.
//!
//! Usage: `cargo run --release --example p25_ratet27_capture_u4_long_settling_pitch_sweep -- <host:port>`
