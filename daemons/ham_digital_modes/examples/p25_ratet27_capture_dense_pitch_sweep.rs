...
//! Focused follow-up to the `g0` pitch-quantizer-shape lead in
//! `AMBE_CHIP_VALIDATION_FINDINGS.md` section 23: denser frequency sampling *strictly within*
//! AMBE's own documented 57-444Hz voice-pitch range (every ~20Hz), averaging multiple converged
//! frames per frequency, to settle whether `g0`'s decoded Golay data really tracks pitch smoothly
//! in-range or whether the earlier 0.897 Spearman correlation was mostly driven by out-of-range
//! frequencies destabilizing the vocoder generally.
//!
//! Usage: `cargo run --release --example p25_ratet27_capture_dense_pitch_sweep -- <host:port>`
