...
#![allow(dead_code)]
//! Structure of the chip's (deterministic after PKT_INIT) unvoiced noise: is the excitation independent of gain, pitch and
...
fn capture(sock: &UdpSocket, buf: &mut [u8; 1024], frame: u128, n: usize) -> Vec<Vec<f64>> {
    configure(sock, buf);
    decode(sock, buf, &vec![frame; n])
}
...
fn main() {
    ...
    let _cases: Vec<(&str, u128)> = vec![
        ...
        ("D: b0=70 other shape 400/20", mk(70, 12, 400, 20)),
        ("E: b0=70 gain 12 shape 200/60 again", mk(70, 12, 200, 60)),
    ];
    // Voiced waveform: after INIT, does our voiced synthesis match the chip's at sample level?
    let voiced_frame = a2_build(&A2Raw { b0: 70, b1: 31, b2: 12, b3: 200, b4: 60, b5: 6, b6: 6, b7: 6, b8: 3 });
    let chip = capture(&sock, &mut buf, voiced_frame, 6);
    let mut dec = AmbePlus2SynthesisDecoder::new();
    for (i, c) in chip.iter().enumerate() {
        let ours = dec.decode_frame(voiced_frame).unwrap().to_vec();
        ...
        for lag in -100..=100 {
            ...
            for k in 0..160i32 {
                ...
                let (x, y) = (c[k as usize], ours[j as usize]);
                sab += x * y;
                saa += x * x;
                sbb += y * y;
            }
            let cc = sab / (saa * sbb).sqrt().max(1e-12);
            if cc > best.0 {
                best = (cc, lag);
            }
        }
        let rms = |v: &[f64]| (v.iter().map(|x| x * x).sum::<f64>() / v.len() as f64).sqrt();
        println!("voiced frame {i}: sample correlation chip vs ours {:.3} at lag {} (rms chip {:.0}, ours {:.0})", best.0, best.1, rms(c), rms(&ours));
    }
}
